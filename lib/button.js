...
function create(window, opts) {
    ...
    // INVALID tbbContainer.setAttribute("label", options.label);
    tbbContainer.setAttribute("tooltiptext", options.tooltiptext);

    tbbButton.setAttribute("id", buttonId);
    // INVALID tbbButton.setAttribute("type", "button");
    ...
    if (!toolbar) {
        var toolbar = toolbarbuttonExists(doc, containerId);
    }
...
exports.moveTo = function(toolbarID) {
    ...
    for (var window of SDK.winUtils.windows(null, { includePrivate: true })) {
        ...
        if (tb) {
            tb.insertItem(containerId, b4, null, false);
...
exports.setIcon = function(tab_id, pathToImage) {
    for (var window of SDK.winUtils.windows(null, { includePrivate: true })) {
        ...
        var button = window.document.getElementById(buttonId);
...
function remove() {
    for (var window of SDK.winUtils.windows(null, { includePrivate: true })) {
        ...
        try {
            ...
            var inPalette = (doc.getElementById("navigator-toolbox") || doc.getElementById("mail-toolbox")).palette.getElementsByAttribute( "id", containerId );
