...
//                request3.get();
    ...
    /**
     * Created when quotes have been (re-)read.
     *
     * @param aUrlProvider
    ...
    const ContentScriptInterface = function(aUrlProvider, anInformationHolder) {
        // ChromeWindow
        const browserWindow = windowUtils.getMostRecentBrowserWindow();
        console.error("browserWindow" + browserWindow);
        // XULElement
        const tabBrowser = tabutils.getTabBrowser(browserWindow);
        console.error("tabBrowser " + tabBrowser);


        try {
            let csp = makeContentScriptParams(null, informationHolder);
            console.error("ContentScriptInterface informationHolder " + informationHolder);
            tabBrowser.contentWindow.postMessage(csp, "*");
        }
        catch(err) {
            console.error(err);
        }

        tabBrowser.addEventListener("DOMContentLoaded", function(event) {
            const reloadPostMessage = function(event2) {
                console.error("event2 " + event2);
                tabBrowser.contentWindow.removeEventListener("load", reloadPostMessage);
                let csp = makeContentScriptParams(null, informationHolder);
                tabBrowser.contentWindow.postMessage(csp, event.target.URL);
            };
            console.error("EVENT tabBrowser DOMContentLoaded");
            console.error("event " + event);
            console.error("ContentScriptInterface informationHolder " + informationHolder);
            tabBrowser.contentWindow.addEventListener("load", reloadPostMessage);
        }, false);

        /*
                try {
                    // console.error("tabBrowser " + tabBrowser);
                    tabBrowser.addEventListener("load", function(e) { console.error("tabBrowser load"); }, false);
                    tabBrowser.addEventListener("DOMContentLoaded", function(event) {
                        console.error("tabBrowser DOMContentLoaded");
                        let csp = makeContentScriptParams(null, informationHolder);
                        console.error("ContentScriptInterface informationHolder " + informationHolder);
                        let _browserWindow = windowUtils.getMostRecentBrowserWindow();
                        let _tabBrowser = tabutils.getTabBrowser(_browserWindow);
                        _tabBrowser.addEventListener("load", function(event) {
                            let _contentWindow = tabutils.contentWindow;
                            _contentWindow.postMessage(csp, event.target.URL);
                        });
                    }, false);
                }
                catch(err) {
                ...
                var contentWindow = tabBrowser.contentWindow;
                try {
                    console.error("contentWindow " + contentWindow);
                    contentWindow.addEventListener("load", function(e) { console.error("contentWindow load"); }, false);
                    contentWindow.addEventListener("DOMContentLoaded", function(e) { console.error("contentWindow DOMContentLoaded"); }, false);
                }
                catch(err) {
                    console.error(err);
                }
        */
        ...
        // eventAggregator.publish("reloadData", csp);


        /*
                windowUtils.getMostRecentBrowserWindow().addEventListener("message", function(event){
                    console.error("getMostRecentBrowserWindow().addEventListener event " + event);
                    console.error("getMostRecentBrowserWindow().addEventListener event.data " + event.data);
                    console.error("getMostRecentBrowserWindow().addEventListener event.origin " + event.origin);
                    console.error("getMostRecentBrowserWindow().addEventListener event.source " + event.source);
                    try {
                        let csp = makeContentScriptParams(null, informationHolder);
                        console.error("getMostRecentBrowserWindow().addEventListener informationHolder " + informationHolder);
                        // tabBrowser.contentWindow.postMessage(csp, event.source);
                        if (event.source)
                            tabBrowser.contentWindow.postMessage(csp, event.source);
                    }
                    catch(err) {
        ...
        */
        ...
        const tabutils = require("sdk/tabs/utils");
        const windowUtils = require("sdk/window/utils");
        const injectContentScripts = function(theDoc) {
        ...
        const injectSettingsScripts = function(theDoc) {
            console.error("injectSettingsScripts " + theDoc);
            console.error(theDoc.head);
        ...
        console.error("ContentScriptInterface");
        // ChromeWindow
        // TODO all windows
        const browserWindow = windowUtils.getMostRecentBrowserWindow();
        console.error("browserWindow" + browserWindow);
        ...
        tabBrowser.addEventListener("DOMContentLoaded", function(event) {
            // HTMLDocument
            console.error("DOMContentLoaded event.target " + event.target);
            //injection.loadIntoContentWindowAndItsFrames(tabBrowser.contentWindow);
            if (event.target.URL === "resource://dcc-sm-at-joint-dot-ax/direct-currency-converter-2-sm/data/settings.html") {
                injectSettingsScripts(event.target);
            }
            else {
                injectContentScripts(event.target);
            }
            console.error("ContentScriptInterface addEventListener DOMContentLoaded    done!");
        }, true);
        tabBrowser.addEventListener("load", function(event) {
            // HTMLDocument
            console.error("load event.target " + event.target);
            // tabBrowser.addEventListener No message received but postMessage works
            // tabBrowser.contentWindow.addEventListener Message received but postMessage won't work later
            // (tabutils.getTabBrowser(windowUtils.getMostRecentBrowserWindow())).contentWindow
            let tempBrowserWindow = windowUtils.getMostRecentBrowserWindow();
            let tempTabBrowser = tabutils.getTabBrowser(tempBrowserWindow);
            let tempContentWindow = tempTabBrowser.contentWindow;
            let csp = makeContentScriptParams(null, informationHolder);
            console.error("tempContentWindow " + tempContentWindow);
            console.error("tempContentWindow.title " + tempContentWindow.title);
            console.error("csp " + csp);
            console.error("informationHolder " + informationHolder);
            console.error("event.target.URL " + event.target.URL);
            // "can't access dead object"
            try {
                // tabBrowser.postMessage is not a function
                tabBrowser.contentWindow.postMessage(csp, event.target.URL);
            }
            catch(err) {
                console.error(err);
            }
            // console.error("tabutils.getURI(tab) " + tabutils.getURI(tab));
            console.error("ContentScriptInterface addEventListener load done!");
        }, true);
        ...
        windowUtils.getMostRecentBrowserWindow().addEventListener("message", function(event){
            console.error("Main received message ");
            console.error(event.data);
            eventAggregator.publish("saveSettings", {
                contentScriptParams: event.data
            });
...
*/
            ...
            showSettingsTab: () => {
                // FIXME const isOpen = settingsWorker != null && settingsWorker.settingsTab != null;
                const isOpen = false;
                if (!isOpen) {
...
*/
                    ...
                    const browserWindow = windowUtils.getMostRecentBrowserWindow();
                    // XULElement
                    const settingsTab = tabutils.openTab(browserWindow, aUrlProvider.getUrl("settings.html"));
...
*/
                    ...
                    const testTab = tabutils.openTab(window, aUrlProvider.getUrl("prices.html"));
                }
                else {
                    // FIXME testPageWorker.testPageTab.activate();
                }
            },
...
/*
 // Firefox
                const setTabs = (aTab) => {
                    ...
                    else if (aTab.url === "resource://dcc-at-joint-dot-ax/direct-currency-converter-2/data/prices.html") {
