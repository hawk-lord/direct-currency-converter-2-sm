...
*/
                    ...
                    const settingsTab = tabutils.openTab(window, aUrlProvider.getUrl("settings.html"));
                    const browser = tabutils.getTabBrowserForTab(settingsTab);
                    browser.addEventListener("DOMContentLoaded", function(event) {
                        ...
                        var win = tabutils.getTabBrowserForTab(settingsTab).contentWindow;
                        injection.loadIntoContentWindowAndItsFrames(win);
                        //const options = {contentScriptFile: [aUrlProvider.getUrl("jquery-2.1.1.min.js"), aUrlProvider.getUrl("jquery-ui-1.10.4.min.js"), aUrlProvider.getUrl("dcc-settings.js"), aUrlProvider.getUrl("dcc-firefox-settings-adapter.js")]};
                        //settingsWorker =  Worker(options, window);
                        //settingsWorker.settingsTab = settingsTab;
                        //settingsWorker.port.emit("showSettings", makeContentScriptParams(aTab, anInformationHolder));
                        win.postMessage(makeContentScriptParams(null, anInformationHolder), "resource://dcc-sm-at-joint-dot-ax/direct-currency-converter-2-sm/data/settings.html");
                        console.error("done!");
                    });
                }
                else {
