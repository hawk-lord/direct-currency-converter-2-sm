...
*/
                    ...
                    // XULElement
                    const tabBrowser = tabutils.getTabBrowserForTab(settingsTab);
/*
                    browserWindow.addEventListener("message", function(event){
                        console.error("Main received message " + event.data);
                        if (event.data === "SettingsAdapter created") {
                            tabBrowser.postMessage(makeContentScriptParams(null, anInformationHolder),
                                "resource://dcc-sm-at-joint-dot-ax/direct-currency-converter-2-sm/data/settings.html");
                        }
                    });
*/
                    tabBrowser.addEventListener("DOMContentLoaded", function(event) {
                        console.error("settingsTab " + settingsTab);
                        ...
                        /*
                         event.target.postMessage is not a function
                         settingsTab.postMessage is not a function
                         tabBrowser.postMessage is not a function
                         browserWindow.postMessage   nothing happens
                         tabBrowser.contentWindow.postMessage   correct

                         console.error("sleep");
                         sleepFor(5000);
                         console.error("slept");
                         */
                        console.error("document " + tabBrowser.contentWindow.document);
                        console.error("document.head " + tabBrowser.contentWindow.document.head);
                        tabBrowser.contentWindow.postMessage(makeContentScriptParams(null, anInformationHolder),
                            "resource://dcc-sm-at-joint-dot-ax/direct-currency-converter-2-sm/data/settings.html");
                        console.error("addEventListener DOMContentLoaded    done!");
                    });
                    tabBrowser.addEventListener("load", function(event) {
                        // TODO check SettingsAdapter is ready before posting
                        tabBrowser.contentWindow.postMessage(makeContentScriptParams(null, anInformationHolder),
                            "resource://dcc-sm-at-joint-dot-ax/direct-currency-converter-2-sm/data/settings.html");
                        console.error("addEventListener load done!");
                    }, true);
                }
                else {
