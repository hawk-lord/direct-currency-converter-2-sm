...
//                request3.get();
    ...
    const StorageServiceProvider = function() {
        ...
        return {
            init: (aDefaultEnabled) => {
                ...
                if (storage.dccPrefs == null) {
                    storage.dccPrefs = {
                        ...
                        thousandSep: ".",
                        showDccActionButton: true,
                        showDccToggleButton: true,
                    ...
                    if (storage.dccPrefs.showDccActionButton == null) {
                        storage.dccPrefs.showDccActionButton = true;
                    }
                    if (storage.dccPrefs.showDccToggleButton == null) {
                        storage.dccPrefs.showDccToggleButton = true;
                    }
            ...
            get showDccActionButton () {
                return storage.dccPrefs.showDccActionButton;
            },
            set showDccActionButton (aShowDccActionButton) {
                storage.dccPrefs.showDccActionButton = aShowDccActionButton;
            },
            get showDccToggleButton () {
                return storage.dccPrefs.showDccToggleButton;
            },
            set showDccToggleButton (aShowDccToggleButton) {
                storage.dccPrefs.showDccToggleButton = aShowDccToggleButton;
            },
    ...
    const InformationHolder = function(aUrlProvider, aStorageService) {
        ...
        // Normal default
        let defaultEnabled = {"SEK":true, "CHF":true, "DKK":true, "EUR":true, "GBP":true, "ISK":true, "JPY":true, "NOK":true, "RUB":true, "USD":true};
        // Test default
      ...
      // format 5: 1.234,56¤
        ...
        return {
            ...
            get showDccActionButton () {
                return aStorageService.showDccActionButton;
            },
            set showDccActionButton (aShowDccActionButton) {
                aStorageService.showDccActionButton = aShowDccActionButton;
            },
            get showDccToggleButton () {
                return aStorageService.showDccToggleButton;
            },
            set showDccToggleButton (aShowDccToggleButton) {
                aStorageService.showDccToggleButton = aShowDccToggleButton;
            },
            setUserCountry: (aUserCountry) => {
    ...
    const ContentScriptInterface = function(aUrlProvider, anInformationHolder) {
        const sendScript = (browserWindow) => {
            // ChromeWindow
            //const browserWindow
            ...
            let tabBrowser = null;
            try {
                tabBrowser = tabutils.getTabBrowser(browserWindow);
                // console.error("tabBrowser " + tabBrowser);
            }
            catch(err) {
                // console.error("tabBrowser " + err);
                // window.getBrowser is not a function
                return;
            }
            if (tabBrowser === null || tabBrowser == null) {
                return;
            }
            ...
            try {
                ...
                // console.error("tabBrowser.contentWindow.location " + tabBrowser.contentWindow.location);
                tabBrowser.contentWindow.postMessage(csp, tabBrowser.contentWindow.location);
                browserWindow.addEventListener("DccResetSettingsEvent", (event) => {
                    // console.error("command " + event.target.getAttribute("command"));
                    eventAggregator.publish("resetSettings");
                }, false, true);
                browserWindow.addEventListener("DccSaveSettingsEvent", (event) => {
                    // console.error("command " + event.target.getAttribute("command"));
                    // console.error("settings " + event.target.getAttribute("settings"));
                    const csp = JSON.parse(event.target.getAttribute("settings"));
                    eventAggregator.publish("saveSettings", {
                        contentScriptParams: csp
                    });
                }, false, true);
            }
            catch(err) {
            ...
            // After DOMContentLoaded, listen for "load" event and then postMessage
            tabBrowser.addEventListener("DOMContentLoaded", (event) => {
                const reloadPostMessage = (event2) => {
                    // console.error("event2 " + event2);
                    // console.error("event2.target.URL " + event2.target.URL);
        ...
        // barsInterface.setButton();
        const allWindows = windowUtils.windows(null, {includePrivate:true});
        allWindows.forEach(sendScript);
        /*
...
*/
                    ...
                    // XULElement
                    settingsTab = tabutils.openTab(browserWindow, aUrlProvider.getUrl("settings.html"));
...
*/
            ...
            setButton: () => {
                ...
                const toggle = function () {
                        anInformationHolder.conversionEnabled = !anInformationHolder.conversionEnabled;
                        eventAggregator.publish("toggleConversion", anInformationHolder.conversionEnabled);
                    };
                ...
                // toolsButton
                // id="toggle-button--dccjointax-direct-currency-converter-action-button"
                button.createButton(eventAggregator, win, {
                    panel: null,
                    onCommand: showSettingsTab,
                    id: "direct-currency-converter-action-button",
                    label: "DCC",
                    tooltiptext: _("Open settings"),
                    show: anInformationHolder.showDccActionButton,
                    image: urlProvider.getUrl("images/1402782691_repair_cost.png")
                });
                // conversionButton
                // id="toggle-button--dccjointax-direct-currency-converter-toggle-button"
                button.createButton(eventAggregator, win, {
                    panel: null,
                    onCommand: toggle,
                    id: "direct-currency-converter-toggle-button",
                    ...
                    show: anInformationHolder.showDccToggleButton,
    ...
    const Controller = function(anEventAggregator) {
        ...
        anEventAggregator.subscribe("saveSettings", (eventArgs) => {
            ...
            const settingsTab = tabsInterface.getSettingsTab();
            if (settingsTab)
                tabutils.closeTab(settingsTab);
        });
...
*/
        ...
        anEventAggregator.subscribe("setButton", () => {
            barsInterface.setButton();
        });
        anEventAggregator.subscribe("dccActionButtonHidden", () => {
            informationHolder.showDccActionButton = false;
        });
        anEventAggregator.subscribe("dccActionButtonVisible", () => {
            informationHolder.showDccActionButton = true;
        });
        anEventAggregator.subscribe("dccToggleButtonHidden", () => {
            informationHolder.showDccToggleButton = false;
        });
        anEventAggregator.subscribe("dccToggleButtonVisible", () => {
            informationHolder.showDccToggleButton = true;
        });
        anEventAggregator.subscribe("newWindow", (eventArgs) => {
            // FIXME if contentScriptInterface is null
            if (contentScriptInterface) {
                contentScriptInterface.sendScript(eventArgs.domWindow);
            }
        });
        this.loadStorage = () => {
            storageService.init(informationHolder.getDefaultEnabled());
